...
export default function AvailabilityCalendar({ title, subtitle, slots }: Props) {
  ...
  return (
    <section style={{ marginTop: 18, border: '1px solid #e2d0fb', borderRadius: 20, padding: 14, background: 'linear-gradient(180deg,#fff,#faf6ff)', boxShadow: '0 10px 24px rgba(125, 93, 183, 0.11)' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 8 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          <button style={{ borderRadius: 999, border: '1px solid #dac7f8', background: '#fff', width: 32, height: 32 }} onClick={() => setCursor(new Date(cursor.getFullYear(), cursor.getMonth() - 1, 1))}>←</button>
